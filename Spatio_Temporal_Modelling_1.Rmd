---
title: "Spatio_Temporal_Modelling_1"
...
## SPATIO-TEMPORAL MODELS. 1990:1999
...
```{r}
years9099 <- c(1990:2022)
data90_99 <- NULL
for(i in years9099){ 
  data90_99 <- rbind(data90_99,select_year_diff2(year = i))
}
data90_99_bench <- data90_99
data90_99[which(data90_99$Year %in% c(2000:2022)),"E3CI_diff"] = NA
```
...
formula_9090_I <- E3CI_diff ~ 1 + GDP_diff + CO2_diff + FA_diff + 
  f(idarea, model = "bym", graph = g) + 
  f(idtime, model = "rw1", hyper = prec.prior) + f(idtime1, model = "iid",
                                                   hyper = prec.prior) +
  f(idareatime, model = "iid")
...
De momento los años mas problematicos son 2014, 2018 y 2020.
...
map2014_predI_e <- ggplot(data = data90_99_bench[rows2014_bench_e,]) +
  geom_sf(aes(fill = fittedI)) + 
  labs(fill = "Bym+Interaction ERA5 - 2014") +
  scale_fill_gradient2(
    midpoint = 0, low = "blue", mid = "white", high = "red") + 
  theme(legend.position = "right",
        panel.grid = element_blank(),
...
```{r}
comb_plot_202122 <- (map2020_real_e +
                       map2021_real_e +
                       map2022_real_e)/(map2020_predI_e +
                                          map2021_predI_e +
                                          map2022_predI_e)
...
```{r}
errors_df9099_t <- data.frame("Likelihood" = rep("t",6),
                              "Model" = c("bym + IID", "bym + rw1", "Interaction I",
                                        "Interaction II","Interaction III","Interaction IV"), 
               "WAIC" = WAICs9099,
               "MLIK" = MLIKs9099,
